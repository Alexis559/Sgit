...
object Commit {
  ...
  def commit(repository: Repository, messageCommit: String, treeIndex: Map[String, Any]): String = {
    ...
    Tree.writeTree(repository, treeIndex) match {
    ...
    val pathCommit = Object.getObjectFilePath(repository, shaCommit)
        IO.readContentFile(pathCommit) match {
          ...
          case Right(contentCommit) =>
            ...
            val listBlob = contentCommit.map(x => x.split(" "))
              .filter(x => x(0) == "blob")
              .map(x => x(2) -> x(1))
              .toMap

            val listTree = contentCommit.map(x => x.split(" "))
              .filter(x => x(0) == "tree")
   ...
   */
  def commitToList(commit: Map[String, Any]): List[String] = {
    commitMapToListRec(commit)
  }
